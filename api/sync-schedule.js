...
async function loadSchedule(code){
  const syncCode = prettyCode(code);
  if(!syncCode){
  ...
  const rows = await supabaseFetch(`${TABLE}?sync_code=eq.${encodeURIComponent(syncCode)}&select=sync_code,payload,label,updated_at&limit=1`, {
    method:"GET"
  });
...
async function saveSchedule({code, data, label}){
  ...
  if(Buffer.byteLength(json, "utf8") > MAX_PAYLOAD_BYTES){
    const error = new Error("This timetable is too large to sync right now. Export JSON instead.");
    error.status = 413;
    throw error;
  }
